...
<header class="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-5">
    <div>
        <h1 class="font-display-lg text-display-lg hidden md:block text-on-surface">Create a New Recipe</h1>
        <h1 class="font-display-lg text-display-lg md:hidden text-on-surface">Create Recipe</h1>
        <p class="font-body-lg text-body-lg text-on-surface-variant mt-1 max-w-2xl">Capture your culinary creation. Fill in the details below to save your recipe to the keeper.</p>
    </div>
    <div class="flex gap-3 shrink-0">
        ...
        <button type="submit" form="recipe-form" class="font-label-md text-label-md px-6 py-3 rounded-lg bg-primary text-on-primary shadow-sm hover:bg-surface-tint transition-colors">Save Recipe</button>
    </div>
</header>

<form id="recipe-form" action="{{ route('recipes.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
    @csrf
            ...
            <div class="flex flex-col gap-5">
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <div id="image-dropzone" class="border-2 border-dashed border-outline-variant rounded-xl p-8 flex flex-col items-center justify-center text-center bg-surface hover:bg-surface-container-low transition-colors cursor-pointer group relative">
                        ...
                        <img id="image-preview" class="hidden max-h-48 rounded-lg object-cover" alt="Preview">
                        <button type="button" id="image-remove" class="hidden absolute top-2 right-2 p-1 rounded-full bg-error text-on-error hover:bg-error/80 transition-colors">
    ...
    <aside class="lg:col-span-4 flex flex-col gap-4 lg:sticky lg:top-24">
        ...
        <div class="hidden lg:flex flex-col gap-3 mt-2">
            <button type="submit" class="font-label-md text-label-md w-full py-3 rounded-lg bg-primary text-on-primary shadow-sm hover:bg-surface-tint transition-colors flex justify-center items-center gap-2">
                <span class="material-symbols-outlined" style="font-size: 18px;">save</span> Save Recipe
            </button>
